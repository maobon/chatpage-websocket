    ...
    <>
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-16 w-full max-w-3xl items-center gap-3 px-5">
          <button
            onClick={() => authSession && setIsAvatarModalOpen(true)}
            disabled={!authSession}
            className={`flex h-9 w-9 items-center justify-center rounded-xl bg-slate-950 text-white overflow-hidden transition-transform active:scale-95 ${authSession ? "cursor-pointer hover:ring-2 hover:ring-slate-200" : ""
              }`}
          >
            {authSession?.avatar ? (
              <img
          ...
          <div>
            <h1 className="font-semibold text-slate-900">Chat Room</h1>
